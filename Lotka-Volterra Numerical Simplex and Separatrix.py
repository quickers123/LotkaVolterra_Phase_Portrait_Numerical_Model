...
# -*- coding: utf-8 -*-
"""
Phase–plane visualisation and polynomial fits (separatrix & carrying simplex)
for a two-species competitive Lotka–Volterra system.

Functional code paths are unchanged; this version standardises formatting,
adds concise docstrings, and polishes comments/labels for readability.
"""
...
# Silence NumPy gradient edge warnings that can appear when x-spacing becomes tiny.
...
def _fmt(x) -> str:
    """Format numbers for terminal prints: 6 sig figs, scientific when needed."""
    try:
        return f"{float(x):.6g}"
    except Exception:
        return str(x)
...
def f(N1: np.ndarray, N2: np.ndarray) -> np.ndarray:
    return N1 * (3.0 - N1 - 1.5 * N2)


def g(N1: np.ndarray, N2: np.ndarray) -> np.ndarray:
...
def jacobian(N1: float, N2: float) -> np.ndarray:
    df_dN1 = 3.0 - 2.0 * N1 - 1.5 * N2
    df_dN2 = -1.5 * N1
    dg_dN1 = -2.5 * N2
    dg_dN2 = 4.0 - 2.0 * N2 - 2.5 * N1
    return np.array([[df_dN1, df_dN2], [dg_dN1, dg_dN2]])
...
# ----------------------------------------------------------------------
# Fourth–order Runge–Kutta steppers and integration utilities
# ----------------------------------------------------------------------

def rk4_step_unit(Y: np.ndarray, h: float) -> np.ndarray:
    """One RK4 step along the *normalised* vector field (arc-length parameter)."""
    def F(Z: np.ndarray) -> np.ndarray:
        V = np.array([f(Z[0], Z[1]), g(Z[0], Z[1])], dtype=float)
...
def integrate_until_axes_unit(
    ...
    """
    Backward tracing along the *normalised* vector field (arc-length parameter).
    ...
    for _ in range(max_steps):
        ...
        if (Y[1] < tol_axis and abs(Y[0] - 3.0) < 5e-2) or (
            Y[0] < tol_axis and abs(Y[1] - 4.0) < 5e-2
        ):
            break
...
def rk4_step(fun: Callable, t: float, Y: np.ndarray, h: float) -> np.ndarray:
    k1 = fun(t, Y)
...
def integrate_manifold(
    Y0: Tuple[float, float], t0: float, t_end: float, h: float, box_max: float = 6.0
) -> Tuple[np.ndarray, np.ndarray]:
    if t_end == t0:
...
def separatrix_diagnostics(N1s: np.ndarray, N2s: np.ndarray) -> Dict[str, float]:
    """
    Stable diagnostics on y(x): de-duplicate x, use 1st-order gradients, and guard divisions.
    Returns a dict with RMS invariance residual plus slope/curvature summaries.
    """
    if N1s.size < 5:
...
def anchor_polys():
    ...
    A = np.poly1d(
        [1.0, -(x0 + xs + x3), (x0 * xs + x0 * x3 + xs * x3), -x0 * xs * x3]
    )
    A1 = np.polyder(A, 1)
    A2 = np.polyder(A, 2)
...
def build_poly_and_derivs(coeff_free: np.ndarray):
...
def fit_carrying_simplex_poly(
    deg: int = 8,
    n_grid: int = 500,
    lam_shape: float = 120.0,
    lam_smooth: float = 1e-7,
    seed: int = 2,
):
    ...
    def objective(cfree: np.ndarray) -> float:
        ...
        # Invariance residual (graph condition): g - y' * f
        R = gi - yp(xi) * fi
        J_inv = np.sum(R * R)
    ...
    # Recover full power-basis coefficients a_k for y(x) by LS on a fine grid
...
if __name__ == "__main__":
    ...
    desmos = "y=" + "+".join(
        (f"{a_power[0]:.12g}",)
        + tuple(f"{a_power[k]:.12g}*x^{k}" for k in range(1, len(a_power)))
    )
    ...
    #           simple poly (deg 9), and anchor-constrained poly (deg 8)
    # -------------------------------------------------------
    ...
    ax.plot(
        N1_vals_plot,
        (3.0 - N1_vals_plot) / 1.5,
        "g--",
        lw=1.5,
        label=r"$N_1$ nullcline",
        zorder=2,
    )
    ax.plot(
        N1_vals_plot,
        4.0 - 2.5 * N1_vals_plot,
        "r--",
        lw=1.5,
        label=r"$N_2$ nullcline",
        zorder=2,
    )
    ...
    added_label = False
    for (x0_seed, y0_seed) in seeds:
        # Forward in time
        N1_f, N2_f = integrate_manifold(
            [x0_seed, y0_seed], t0=0.0, t_end=t_span, h=h_traj, box_max=box_bound
        )
        # Backward in time
        N1_b, N2_b = integrate_manifold(
            [x0_seed, y0_seed], t0=0.0, t_end=-t_span, h=h_traj, box_max=box_bound
        )
    ...
    ax.plot(
        N1_true, N2_true, color="tab:blue", lw=2.2, label="numerical separatrix", zorder=4
    )

    # Simple polynomial LSQ (deg 9)
    ax.plot(
        N1_grid_simple,
        N2_poly_simple,
        "k-",
        lw=2.0,
        label=f"P({poly_degree}) LSQ Separatrix",
        zorder=5,
    )
    ...
    ax.plot(
        ...
        color="#FF7F0E",
        lw=2.4,
        label=f"P({DEG}) LSQ Carrying Simplex",
        zorder=6,
    )

    # Equilibria
    ax.plot([0, 3, 0, N1_star], [0, 0, 4, N2_star], "ko", ms=5, zorder=7)
    ...
    ax.set_ylabel(r"$N_2$")
    ax.set_title(
        "Lotka–Volterra Phase Plane with Numerical LSQ fits "
        "of Separatrix & Carrying Simplex Curves."
    )
